...
// === ROUTE 1: LOGIN (NOW WITH LOCK) ===
app.get('/login', (req, res) => {
    const sl_uuid = req.query.uuid;
    
    if (!sl_uuid) {
        return res.send("ERROR: Missing UUID. Use the HUD.");
    }
    
...
    
   
    // The 'true' at the end FORCES Spotify to show the login/confirmation screen
    // This prevents Avatar B from accidentally logging into Avatar A's account
...
        } catch (err) 
        {
            return res.json({ track: `Session Error`, artist: 'Relog the HUD', error_code: "REFRESH_ERROR" });
        }
    }
