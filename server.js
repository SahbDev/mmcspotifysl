// server.js (VERSÃO BLINDADA CONTRA ERRO 500)
...
app.get('/login', (req, res) => {
  ...
  const options = { state: 'state', showDialog: true };
...
app.get('/tocando', async (req, res) => {
  ...
  // Se não tiver token (usuário novo ou resetado), retorna "false" sem dar erro
  ...
  try {
    ...
    if (err.statusCode === 401) {
      try {
        ...
        // Token expirou e não renovou? Retorna desconectado (não erro 500)
        res.json({ tocando: false });
      }
    } else {
      // Qualquer outro erro, apenas diz que não está tocando para o SL não travar
      console.error(err);
      res.json({ tocando: false });
    }
  }
...
// ROTAS DE CONTROLE
app.post('/play', async (req, res) => { try { await spotifyApi.play(); res.status(200).send('OK'); } catch (err) { res.status(200).send('OK'); } });
app.post('/pause', async (req, res) => { try { await spotifyApi.pause(); res.status(200).send('OK'); } catch (err) { res.status(200).send('OK'); } });
app.post('/next', async (req, res) => { try { await spotifyApi.skipToNext(); res.status(200).send('OK'); } catch (err) { res.status(200).send('OK'); } });
app.post('/previous', async (req, res) => { try { await spotifyApi.skipToPrevious(); res.status(200).send('OK'); } catch (err) { res.status(200).send('OK'); } });
app.post('/revoke', (req, res) => { spotifyApi.setAccessToken(null); spotifyApi.setRefreshToken(null); res.status(200).send('Revoked'); });
